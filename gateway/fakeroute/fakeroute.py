...
DST_INTERFACE = None # < Internal network interface, to fix : use command line passed argument
...
class LocalSpoofer:
    ...
    def __init__(self):
        self.sockets = {
            4: socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.IPPROTO_RAW),
            6: socket.socket(socket.AF_PACKET, socket.SOCK_RAW, socket.IPPROTO_RAW)
        }

    def spoof(self, packet, ip_version = 4):
        #version, _, dst, _ = get_packet_info(packet)
        print("LocalSpoofer::spoof for v=" + str(ip_version) +
          ", packet=" + " ".join("{:02x}".format(c) for c in packet) + ", p=" + str(packet),
          flush=True)
        self.sockets[ip_version].sendto(packet, (str(DST_INTERFACE), 0))
...
class TracerouteFakeTarget:
    ...
    def spoof(self, payload):
        ...
        logging.debug("Spoof IPv" + str(version) + " packet from " + str(spoof_addr) +
          " to " + str(src) + ", ttl = " + str(ttl))
        if version == 4:
            packet = Ether() / IP(src=spoof_addr, dst=str(src)) / ICMP(type=0xb, code=0) / payload
        else:
            packet = Ether() / IPv6(src=spoof_addr, dst=str(src)) / ICMPv6TimeExceeded() / payload
        self.spoofer.spoof(bytes(packet), ip_version = version)
    ...
    def run(self):
        ...
        logging.debug('TracerouteFakeTarget: to recv loop, sockets = ' + str(len(sockets)))
        while True:
            read, _, _ = select.select(sockets, [], [])
            for r in read:
                payload, _ = r.recvfrom(0xffff) # payload : IP/...
                # Catch all packets with ttl inside 1..n (traceroute use UDP(port 33434), tracert ICMP)
                version, src, dst, ttl = get_packet_info(payload)
                n = self.num_addresses(version)
                logging.debug('TracerouteFakeTarget: received packet from ' + str(src) + ' to ' + str(dst) +
                  ", ttl = " + str(ttl) + ", n = " + str(n))
                if 1 <= ttl <= n:
                  self.spoof(payload)
...
def main():
    ...
    parser.add_argument('--spoofer-iface', help='Interface for send spoof packets', default='eth1')
    ...
    global DST_INTERFACE
    DST_INTERFACE = args.spoofer_iface
    key = None
